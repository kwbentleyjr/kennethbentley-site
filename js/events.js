...
async function loadEvents(options = {}) {
  ...
  try {
    ...
    const visibleEvents = options.limit ? upcoming.slice(0, options.limit) : upcoming;

    if (visibleEvents.length === 0) {
      container.innerHTML = "<p>No upcoming shows listed right now.</p>";
      return;
    }
    ...
    container.innerHTML = "<p>Events could not be loaded.</p>";
    console.error(error);
  }
}
